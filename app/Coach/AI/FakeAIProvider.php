...
class FakeAIProvider implements AIProviderInterface
{
    ...
    {
        if($request->promptKey==='certification.skills_gap')return ['target_role'=>"[{$request->language}] Poste cible déclaré",'current_strengths'=>['Force issue du profil à confirmer'],'missing_skills'=>['Compétence suggérée, non acquise'],'skills_to_improve'=>['Compétence déclarée à approfondir'],'priority_skills'=>[['skill'=>'Compétence cible','priority'=>'high','reason'=>'Priorité indicative à valider']],'evidence_from_profile'=>['Élément factuel du profil uniquement']];
        if($request->promptKey==='certification.recommend')return ['recommendations'=>[['name'=>"[{$request->language}] Certification illustrative",'provider'=>'Fournisseur indicatif non vérifié','why_relevant'=>'Pour développer une compétence prioritaire','skills_addressed'=>['Compétence cible'],'priority'=>'high','prerequisites'=>['À vérifier auprès du fournisseur'],'notes'=>'Suggestion IA à vérifier; disponibilité et contenu non confirmés']],'training_suggestions'=>['Type de formation pratique à rechercher sans connexion LMS']];
        if($request->promptKey==='certification.learning_plan')return ['goal'=>"[{$request->language}] Développer progressivement les compétences cibles",'steps'=>[['title'=>'Valider le besoin','skills'=>['Compétence cible'],'priority'=>'high','suggested_duration'=>'2 semaines','recommended_resources'=>['Ressource générique à vérifier']],['title'=>'Pratiquer et documenter','skills'=>['Compétence cible'],'priority'=>'medium','suggested_duration'=>'4 semaines','recommended_resources'=>['Projet pratique']]]];
