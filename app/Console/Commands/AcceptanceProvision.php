...
class AcceptanceProvision extends Command
{
    ...
    protected $description = 'Prépare le dataset contrôlé de recette local/staging';

    public function handle(AcceptanceDatasetService $service): int
    {
        if ($this->option('apply') && $this->option('dry-run')) {
            $this->error('Choisir --dry-run ou --apply.');
        ...
        try {
            ...
            if (! $this->option('apply')) {
                ...
                return ($plan['dependencies'] || $plan['conflicts']) ? self::FAILURE : self::SUCCESS;
            }
            $manifest = $service->provision($plan);
            $this->info('Provisioning réussi. Run-id : '.$manifest['run_id']);
