...
class AcceptanceCleanup extends Command {
 ...
 public function handle(AcceptanceDatasetService $service):int{
  ...
  try {$manifest=$service->cleanupPlan((string)$this->argument('run-id'));
   ...
   if(!$this->option('apply')){$this->info('DRY-RUN cleanup : aucune suppression.');return self::SUCCESS;}
   $service->cleanup($manifest['run_id']);$this->info('Cleanup exact réussi.');return self::SUCCESS;
