...
class CoachConversationController extends Controller
{
    ...
    public function destroy(Request $request,int $conversation) { $model=$request->user()->coachConversations()->findOrFail($conversation); $this->authorize('delete',$model); $model->delete(); return redirect()->route('coach.dashboard'); }
}
