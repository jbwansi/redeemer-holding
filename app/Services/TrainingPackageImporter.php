...
{
    public function __construct(private readonly TrainingPackageAnalyzer $analyzer, private readonly TrainingJsonImporter $jsonImporter, private readonly TrainingJsonUpdateApplier $updateApplier) {}
    ...
    public function import(string $path, string $mode, string $filename = ''): array
    {
        ...
        if (($package['analysis']['status'] ?? null) !== $expectedStatus) {
            throw new DomainException($mode === 'create' ? 'Le package ne correspond pas à une nouvelle formation.' : 'Le package ne cible pas une formation existante.');
        }
        $zip = new ZipArchive;
