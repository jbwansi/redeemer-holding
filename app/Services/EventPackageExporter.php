...
{
    public function __construct(private readonly EventJsonExporter $jsonExporter, private readonly EventPackageMediaReferences $mediaReferences) {}

    public function export(Event $event): string
    {
        $package = $this->jsonExporter->package($event);
        $temporary = tempnam(sys_get_temp_dir(), 'event-package-');
        if ($temporary === false) {
        ...
        try {
            $zip->addFromString('event.json', json_encode($package, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR).PHP_EOL);
            $included = [];
            $missing = [];
            $archiveHashes = [];
            ...
            $manifest = [
                'package_version' => '1.0', 'type' => 'event-package',
                'schema_version' => $package['schema_version'], 'event_slug' => $package['data']['slug'],
                'media' => ['included' => count($included), 'missing' => count($missing), 'files' => $included, 'missing_files' => $missing],
            ];
