    ...
    <div class="container">
        ...
        <div class="header">
            <h1>
                Rappel :
                @if (($daysBefore ?? 1) <= 1)
                     Votre {{ $label }} commence demain
                @else
                    Votre {{ $label }} commence dans {{ $daysBefore }} jours
                @endif
            </h1>
        </div>

        <div class="content">
           <p>Bonjour {{ $participant->name ?? $participant->first_name }},</p>

            @if (!empty($customMessage))
                <p>{{ $customMessage }}</p>
            @else
                <p>
                    Nous vous rappelons que le {{ $label }} "{{ $participant->event->title }}"
                    @if (($daysBefore ?? 1) <= 1)
                        commence demain.
                    @else
            ...
            <div class="details">
                ...
                <ul>
                    <li>Date : {{ $participant->event->start_date->format('d/m/Y') }}</li>
                    <li>Heure : {{ $participant->event->start_date->format('H:i') }}</li>
                    <li>Lieu : {{ $participant->event->location }}</li>
