...
const TrainingsPage = ({ trainings, featuredTraining, pageContent = {} }: any) => {
  ...
  const lastPage = trainings?.meta?.last_page ?? 1;
  const now = new Date();
  ...
  const activeFeatured = useMemo(() => {
    const candidate = featuredTraining || upcoming?.[0] || null;
          ...
          {filteredUpcoming.length === 0 ? (
            ...
            <div className="space-y-6">
              {filteredUpcoming.map((formation: any) => (
                ...
                >
                  <TrainingCard formation={formation} />
                </div>
              ))}
            </div>
          )}
            ...
            <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
              <div>
                ...
                <h3 className="mt-3 text-3xl font-black">
                  {pageContent.final_cta_title ||
                    'On vous recommande la meilleure formation selon votre objectif'}
                </h3>

                {pageContent.final_cta_text && (
                  <p className="mt-3 max-w-2xl text-white/85">{pageContent.final_cta_text}</p>
                )}
              </div>
