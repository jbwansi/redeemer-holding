...
const ClientFormations = ({ formations }: any) => {
    ...
    const statCounts = useMemo(() => {
        const now = new Date();
        const ongoing = allFormations.filter((f: Formation) => now >= new Date(f.start_date) && now <= new Date(f.end_date)).length;
        const upcoming = allFormations.filter((f: Formation) => new Date(f.start_date) > now).length;
        const past = allFormations.filter((f: Formation) => new Date(f.end_date) < now).length;
        return { all: allFormations.length, ongoing, upcoming, past };
    }, [allFormations]);

    const filteredFormations = useMemo(() => {
        const currentDate = new Date();
        return allFormations.filter((formation: Formation) => {
    ...
    }, [allFormations, searchTerm, statusFilter]);

    const statusOptions = [
        { key: FormationStatus.ALL, label: 'Toutes', count: statCounts.all },
        { key: FormationStatus.ONGOING, label: 'En cours', count: statCounts.ongoing },
        { key: FormationStatus.UPCOMING, label: 'A venir', count: statCounts.upcoming },
        { key: FormationStatus.PAST, label: 'Passees', count: statCounts.past },
    ];
            ...
            <div className="space-y-6">
                ...
                >
                    ...
                    <div className="relative">
                        ...
                        <p className="mt-2 text-sm md:text-base text-white/80 max-w-2xl">
                            Suivez vos formations en cours, les sessions a venir et votre historique depuis une interface claire.
                        </p>
                    </div>
                ...
                >
                        ...
                        <div className="flex flex-wrap gap-2">
                            {statusOptions.map((status) => (
                                <button
                                    ...
                                    className={`inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-sm transition-all duration-200 ${statusFilter === status.key
                                        ? 'bg-red-600 text-white shadow-md shadow-red-500/20'
                                        : 'bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700'
                                        }`}
                                >
                                    <span>{status.label}</span>
                                    <span className={`rounded-full px-2 py-0.5 text-xs ${statusFilter === status.key ? 'bg-white/20' : 'bg-white dark:bg-slate-900/60'}`}>
                                        {status.count}
                                    </span>
                                </button>
                            ))}
                    ...
                    <div className="mt-3 text-sm text-slate-600 dark:text-slate-400">
                        {filteredFormations.length} formation{filteredFormations.length > 1 ? 's' : ''} trouvee{filteredFormations.length > 1 ? 's' : ''}
                    </div>
                </motion.div>
                ...
                <div className="space-y-6">
                    {filteredFormations.length === 0 ? (
                        ...
                        filteredFormations.map((formation: Formation, index: number) => (
                            ...
                            >
                                <DashboardFormationCard formation={formation} />
                            </motion.div>
                        ))
                    )}
                </div>
