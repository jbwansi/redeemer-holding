...
import { Calendar, ChevronLeft, Clock, GraduationCap, MapPin, Users, ArrowRight, CheckCircle } from 'lucide-react';
...
const FormationDetailPage = ({ formation }: any) => {
    const { auth } = usePage().props as any;
    const safeContent = useMemo(() => DOMPurify.sanitize(formation?.content || ''), [formation?.content]);

    const now = new Date();
    const endDate = formation?.end_date ? new Date(formation.end_date) : null;
    const isPast = endDate ? endDate < now : false;
    const canRegister = !isPast && !formation?.is_full && (formation?.available_seats ?? 0) > 0;
        ...
        <FrontLayout>
            ...
            <main className="relative min-h-screen overflow-hidden bg-[#f7f6f2] pt-24 pb-20 dark:bg-slate-950">
                ...
                <section className="mx-auto max-w-[1320px] px-6 md:px-8">
                    <Link href={route('formations')} className="inline-flex items-center gap-1.5 text-sm text-slate-600 hover:text-[#0f766e] dark:text-slate-300">
                    ...
                    <div className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
                        <div className="relative">
                            <img src={resolveImage(formation?.featured_image)} alt={formation?.title} className="h-[380px] w-full object-cover" />
                            <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/40 to-transparent" />

                            <div className="absolute bottom-0 left-0 z-10 w-full p-7 md:p-10">
                                <span className="inline-flex rounded-full bg-[#0f766e] px-3 py-1 text-xs font-medium text-white">Formation</span>
                                <h1 className="mt-4 max-w-3xl text-3xl font-semibold text-white md:text-5xl">{formation?.title}</h1>
                                <p className="mt-3 max-w-2xl text-white/85">{formation?.excerpt}</p>
                            </div>
                        </div>
                        ...
                        <div className="grid gap-8 p-7 md:grid-cols-3 md:p-10">
                            ...
                            <aside className="space-y-4">
                                <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
                                    ...
                                    <p className="mt-2 inline-flex items-center gap-2 text-sm"><Calendar className="h-4 w-4" />{formatDate(formation?.start_date)} - {formatDate(formation?.end_date)}</p>
                                </div>

                                <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
                                    <p className="text-xs uppercase tracking-wide text-slate-500">Horaire</p>
                                    <p className="mt-2 inline-flex items-center gap-2 text-sm"><Clock className="h-4 w-4" />{formatTime(formation?.start_date)} - {formatTime(formation?.end_date)}</p>
                                </div>

                                <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
                                    <p className="text-xs uppercase tracking-wide text-slate-500">Lieu</p>
                                    <p className="mt-2 inline-flex items-center gap-2 text-sm"><MapPin className="h-4 w-4" />{formation?.location || '-'}</p>
                                </div>
                                ...
                                <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
                                    ...
                                    <p className="mt-2 inline-flex items-center gap-2 text-sm">
                                        ...
                                        {formation?.max_participants ? `${formation?.available_seats} disponibles` : 'Illimitees'}
                                    </p>
                                    {formation?.max_participants && (formation?.available_seats ?? 0) <= 5 && (formation?.available_seats ?? 0) > 0 && (
                                ...
                                <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
                                    ...
                                    <p className="mt-2 text-xl font-semibold text-[#0f766e]">{formation?.price > 0 ? `${formation.price} CHF` : 'Gratuit'}</p>
                                </div>
                ...
                <section id="inscription" className="mx-auto mt-12 max-w-[1320px] px-6 md:px-8">
                    {canRegister ? (
                        <FormationJoin formation={formation} auth={auth} />
                    ) : (
                        <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center dark:border-slate-700 dark:bg-slate-900">
                            <h3 className="text-xl font-semibold text-slate-900 dark:text-white">Inscription indisponible</h3>
                            <p className="mt-2 text-slate-600 dark:text-slate-300">
                                {isPast ? 'Les inscriptions sont fermees car la formation est terminee.' : 'Les places sont actuellement completes.'}
                            </p>
                            <Link href={route('contact')} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-[#0f766e] px-5 py-3 text-sm font-medium text-white">
                    ...
                    <div className="rounded-3xl bg-gradient-to-r from-[#0f766e] to-[#115e59] p-10 text-white md:p-12">
                        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
                            <div>
                                ...
                                <h3 className="mt-3 text-3xl font-semibold">Parlons de votre parcours de progression</h3>
                            </div>
