...
import { AlertCircle, ArrowLeft, Calendar, Clock, CreditCard, GraduationCap, Monitor, Shield } from 'lucide-react';
        ...
        <FrontLayout>
            ...
            <main className="relative min-h-screen overflow-hidden bg-[#f7f6f2] pt-24 pb-20 dark:bg-slate-950">
                ...
                <section className="mx-auto max-w-[1200px] px-6 md:px-8">
                    <Link href={route('formations.details', formation.slug)} className="inline-flex items-center text-sm text-slate-600 hover:text-[#0f766e] dark:text-slate-300">
                        <ArrowLeft className="mr-2 h-4 w-4" />
                        Retour a la formation
                    </Link>

                    <h1 className="mt-5 text-3xl font-semibold text-slate-900 md:text-4xl dark:text-white">Finaliser votre inscription</h1>
                    <p className="mt-2 text-slate-600 dark:text-slate-300">Validez votre paiement pour acceder au programme.</p>
                    ...
                    <div className="mt-8 grid gap-6 lg:grid-cols-5">
                        <div className="lg:col-span-3 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                            <div className="flex flex-col gap-5 md:flex-row">
                                ...
                                <div>
                                    <h2 className="text-xl font-semibold text-slate-900 dark:text-white">{formation?.title}</h2>
                                    <div className="mt-4 space-y-2 text-sm text-slate-600 dark:text-slate-300">
                                        <p className="inline-flex items-center gap-2"><Calendar className="h-4 w-4" />{formatDate(formation?.start_date)}</p>
                                        <p className="inline-flex items-center gap-2"><Clock className="h-4 w-4" />{formation?.duration || '-'} heures</p>
                            ...
                            <div className="mt-6 rounded-2xl bg-slate-50 p-5 dark:bg-slate-800/50">
                                ...
                                <div className="mt-3 space-y-2 text-sm">
                                    <div className="flex justify-between"><span>Formation</span><span>{formatCurrency(subtotal)}</span></div>
                        ...
                        <div className="lg:col-span-2 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                            ...
                            <div className="mt-4 rounded-xl bg-emerald-50 p-4 dark:bg-emerald-900/20">
                                {timeExpired ? (
                                    <div className="text-sm text-amber-700 dark:text-amber-300">
                                        ...
                                        <p className="mt-2">Relancez l'inscription depuis la page formation.</p>
                                    </div>
                                ) : (
                                    <>
                                        <p className="text-sm text-emerald-700 dark:text-emerald-300">Temps restant pour payer</p>
                                        <CountdownTimer expiryTimestamp={expirationTime} onExpire={() => setTimeExpired(true)} className="mt-2 text-2xl font-bold text-emerald-800 dark:text-emerald-200" />
                                    </>
                                )}
                            </div>
                            ...
                            <ul className="mt-5 space-y-2 text-sm text-slate-600 dark:text-slate-300">
                                ...
                                <li className="inline-flex items-center gap-2"><Shield className="h-4 w-4 text-green-600" />Acces active apres validation</li>
                            </ul>
                            ...
                            <button
                                ...
                                className="mt-6 w-full rounded-xl bg-[#0f766e] px-5 py-3 text-sm font-medium text-white transition hover:bg-[#115e59] disabled:cursor-not-allowed disabled:opacity-60"
                            >
