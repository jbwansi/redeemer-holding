            ...
            <main className="relative min-h-screen overflow-hidden bg-[#f7f6f2] pb-20 pt-24 dark:bg-slate-950">
                ...
                <section className="relative z-10 mx-auto mt-12 max-w-[1180px] px-6 md:px-8">
                    <div className="grid overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-xl shadow-slate-900/5 md:grid-cols-4 dark:border-slate-700 dark:bg-slate-900">
                        <Info icon={<Calendar />} label="Date" value={formatEventDate(event.start_date, event.end_date)} />
                        <Info icon={<Clock />} label="Horaire" value={`${formatTime(event?.start_date)} - ${formatTime(event?.end_date)}`} />
                        <Info icon={<MapPin />} label="Lieu" value={event?.location || '-'} />
                        <Info
                            icon={<Users />}
                            label="Places"
                            value={event?.max_participants ? `${event?.available_seats} disponibles` : 'Illimitées'}
                        />
                    </div>
                </section>

                <section id="details" className="mx-auto mt-12 grid max-w-[1320px] gap-8 px-6 md:px-8 lg:grid-cols-[1fr_380px]">
                    ...
                    {canRegister ? (
                        ...
                        <div className="rounded-[2rem] border border-slate-200 bg-white p-10 text-center shadow-xl shadow-slate-900/5 dark:border-slate-700 dark:bg-slate-900">
                            ...
                            <p className="mt-2 text-slate-600 dark:text-slate-300">
                                {isPast ? 'Les réservations sont fermées car l’événement est terminé.' : 'Les places sont actuellement complètes.'}
                            </p>
...
const SideInfo = ({ icon, label, value, highlight = false }: any) => (
    <div className="rounded-2xl border border-slate-200 p-4 transition hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800/50">
        <p className="text-xs font-bold uppercase tracking-wide text-slate-500">
            {label}
        </p>
        <p className={`mt-2 inline-flex items-center gap-2 text-sm font-bold ${highlight ? 'text-[#da2e29]' : 'text-slate-800 dark:text-white'}`}>
