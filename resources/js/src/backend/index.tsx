...
function Dashboard({
  ...
  const sortedVisitorsByDay = useMemo(() => {
    return [...visitorsByDay].sort((a, b) => a.name.localeCompare(b.name));
  }, [visitorsByDay]);
  ...
  const maxHeatmapValue = Math.max(...heatmapData.map((item: any) => Number(item.value)), 1);
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm md:text-base text-white/80 max-w-2xl">
                  Vision claire de votre activite: contenus, performances commerciales et tendances
                  en un seul ecran.
                </p>
              </div>
            ...
            <Card key={index} className={cardClass}>
              ...
              <CardHeader className={cardHeaderClass}>
                <div>
                  ...
                  <p className={cardSubtitleClass}>Total: {card.total}</p>
                </div>
              ...
              <CardContent>
                ...
                <div className="mt-3 flex items-center gap-2">
                  <span
                    className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${card.trend >= 0 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300' : 'bg-rose-100 text-rose-700 dark:bg-rose-500/15 dark:text-rose-300'}`}
                  >
                    {card.trend >= 0 ? (
                  ...
                  <span className={cardSubtitleClass}>evolution mensuelle</span>
                </div>
              </CardContent>
            </Card>
          ))}
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className={cardSubtitleClass}>
                  Surveillance en temps reel du traitement asynchrone
                </p>
              </div>
              <span
                className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${queueStatus.badge}`}
              >
                {queueStatus.label}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              ...
              <div className="rounded-xl border border-slate-200/70 bg-slate-50/60 px-3 py-2.5 dark:border-slate-700/60 dark:bg-slate-800/30">
                ...
                <p className="mt-1 text-xl font-semibold">
                  {queueHealth?.oldest_pending_minutes ?? 0}
                </p>
              </div>
            </div>
            <p className={cardSubtitleClass}>
              Driver: {queueHealth?.driver ?? 'n/a'} | Seuils:{' '}
              {queueHealth?.thresholds?.max_pending ?? 0} pending,{' '}
              {queueHealth?.thresholds?.max_failed ?? 0} failed,{' '}
              {queueHealth?.thresholds?.max_oldest_minutes ?? 0} min
            </p>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <CardContent>
              <div className="space-y-4">
                {(stats.top_content ?? []).map((item: any, index: number) => (
                  <div
                    key={index}
                    className="flex items-center rounded-xl border border-slate-200/70 bg-slate-50/60 px-3 py-2.5 dark:border-slate-700/60 dark:bg-slate-800/30"
                  >
          ...
          <Card className={cardClass}>
            <CardHeader className={cardHeaderClass}>
              ...
              <p className={cardSubtitleClass}>Utilisateurs actifs sur les 30 derniers jours</p>
            </CardHeader>
          ...
          <Card className={cardClass}>
            <CardHeader className={cardHeaderClass}>
              ...
              <p className={cardSubtitleClass}>Conversion par événement actif ou à venir</p>
            </CardHeader>
          ...
          <Card className={cardClass}>
            <CardHeader className={cardHeaderClass}>
              ...
              <p className={cardSubtitleClass}>Pics de trafic sur 30 jours</p>
            </CardHeader>
          ...
          <Card className={cardClass}>
            <CardHeader className={cardHeaderClass}>
              ...
              <p className={cardSubtitleClass}>Analyse des performances</p>
            </CardHeader>
