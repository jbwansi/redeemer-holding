...
const ShowFormation = ({ formation }: { formation: Formation }) => {
    const safeContent = React.useMemo(() => DOMPurify.sanitize(formation.content || ''), [formation.content]);
            ...
            <div className="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-gradient-to-br from-slate-950 via-slate-900 to-[#7f1d1d] px-6 py-7 text-white shadow-xl">
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                    <div>
                        ...
                        <h1 className="text-2xl md:text-3xl font-bold tracking-tight mt-1">{formation.title}</h1>
                        <p className="mt-2 text-white/80 max-w-2xl">{formation.excerpt}</p>
                    </div>
                    ...
                    <Card className="border-slate-200/80 bg-white/95 shadow-sm dark:border-slate-700/60 dark:bg-slate-900/70">
                        <CardContent className="p-6 space-y-6">
                            <div className="flex items-center justify-between">
                                <span className="text-2xl font-bold text-slate-900 dark:text-slate-100">{parseInt(formation.price).toLocaleString()} CHF</span>
                                <Badge variant="outline" className="text-xs">{formation.is_full ? 'Complet' : 'Ouvert'}</Badge>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-slate-600 dark:text-slate-400">{formation.location}</p>
                                    </div>
                                </div>
