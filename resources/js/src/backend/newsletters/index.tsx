...
export default function NewsletterIndex({
  ...
  const toggleSegment = (key: SegmentKey, checked: boolean) => {
    ...
    setData(
      'segments',
      current.filter((segment) => segment !== key)
    );
  };
  ...
  const onSubmit = (event: React.FormEvent) => {
    ...
    post(route('admin.newsletters.send'), {
      ...
      onError: () => {
        toast.error(
          'Impossible d envoyer la newsletter. Verifiez les champs et la configuration SMTP.'
        );
      },
    });
  };
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Envoyer uniquement a une adresse de test.
                      </p>
                    </div>
                    <input
                      type="checkbox"
          ...
          <Card>
            ...
            <CardContent>
              {pendingSubscribers.length === 0 ? (
                <p className="text-sm text-muted-foreground">Aucun abonnement en attente.</p>
              ) : (
                <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
