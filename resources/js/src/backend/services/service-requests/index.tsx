...
import {
    Search, MoreVertical, Trash2, ArrowUpDown, ChevronLeft, ChevronRight,
    Eye, Clock, Mail, Phone, Inbox, AlertCircle, Loader, CheckCircle2, XCircle,
} from 'lucide-react';
...
export default function Index({ requests, stats, filters }: Props) {
    ...
    const updateFilters = (newFilters: Record<string, string>) => {
        router.get(route('service-requests.index'), { ...filters, ...newFilters }, {
    ...
    const handleDelete = (request: ServiceRequest) => {
        toast.warning(`Supprimer la demande de ${request.first_name} ${request.last_name} ?`, {
            action: {
                label: 'Supprimer',
                onClick: () => router.delete(route('service-requests.destroy', request.id), {
    ...
    const kpis = [
        { label: 'Total', value: stats?.total ?? requests.meta.total, icon: Inbox, bg: 'bg-primary/10', color: 'text-primary' },
        { label: 'En attente', value: stats?.pending ?? 0, icon: AlertCircle, bg: 'bg-amber-50 dark:bg-amber-950/30', color: 'text-amber-600' },
        { label: 'En cours', value: stats?.in_progress ?? 0, icon: Loader, bg: 'bg-blue-50 dark:bg-blue-950/30', color: 'text-blue-600' },
        { label: 'Terminées', value: stats?.completed ?? 0, icon: CheckCircle2, bg: 'bg-emerald-50 dark:bg-emerald-950/30', color: 'text-emerald-600' },
    ];
                    ...
                    <CardContent className="p-4">
                        <div className="flex flex-col gap-3 md:flex-row md:items-center">
                            ...
                            <div className="flex gap-3">
                                <Select value={selectedStatus} onValueChange={v => { setSelectedStatus(v); updateFilters({ status: v }); }}>
                                ...
                                <Select value={selectedDate} onValueChange={v => { setSelectedDate(v); updateFilters({ date: v }); }}>
                            ...
                            <TableBody>
                                {requests.data.length === 0 ? (
                                    ...
                                    requests.data.map(request => {
                                        ...
                                        return (
                                            <TableRow key={request.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
                                                <TableCell>
                                                ...
                                                <TableCell>
                                                    <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border font-medium ${st.cls}`}>
