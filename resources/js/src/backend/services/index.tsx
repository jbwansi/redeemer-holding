  ...
  return (
    <div ref={setNodeRef} style={style}>
      <div
        {...attributes}
        {...listeners}
        className="mb-2 cursor-grab text-xs text-muted-foreground"
      >
...
const ServiceCard = ({
  ...
  return (
    ...
    >
      <Card
        className={`relative overflow-hidden border-slate-200/80 bg-white shadow-sm transition-all hover:shadow-md dark:border-slate-700/60 dark:bg-slate-900/70 ${
          service.position ? 'ring-2 ring-[#da2e29]/40' : ''
        }`}
      >
        {service.position && (
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-4 rounded-xl border border-slate-100 bg-slate-50/70 p-3 dark:border-slate-800 dark:bg-slate-800/40">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <Badge variant="outline">{service.cta_primary_label || 'Réserver'}</Badge>

                  {service.cta_secondary_label && (
                    <Badge variant="outline">{service.cta_secondary_label}</Badge>
                  )}
                </div>
                ...
                {(service.cta_primary_url || service.cta_secondary_url) && (
                  <div className="mt-2 space-y-1 text-[11px] text-muted-foreground">
                    {service.cta_primary_url && (
                      <p className="truncate">Principal : {service.cta_primary_url}</p>
                    )}

                    {service.cta_secondary_url && (
                      <p className="truncate">Secondaire : {service.cta_secondary_url}</p>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                <div className="flex items-center gap-1">
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-blue-700" asChild>
                ...
                <Button
                  ...
                  className={`ml-2 gap-1 ${
                    service.position ? 'bg-[#da2e29] text-white hover:bg-[#c62823]' : ''
                  }`}
...
export default function ServicesIndex({ services, stats }: Props) {
  ...
  const [optimisticHomePositions, setOptimisticHomePositions] = useState<
    Record<number, number | null>
  >({});

  const servicesWithOptimisticHome = useMemo(
    () =>
      services.data.map((service) => {
        const override = optimisticHomePositions[service.id];
        return override === undefined ? service : { ...service, position: override };
      }),
    [optimisticHomePositions, services.data]
  );
  ...
  const handleHomeToggle = (service: Service) => {
    const currentPosition =
      optimisticHomePositions[service.id] === undefined
        ? service.position
        : optimisticHomePositions[service.id];
    const isHome = currentPosition !== null;
    const nextPosition = isHome ? null : homeServices.length + 1;

    setOptimisticHomePositions((prev) => ({
      ...prev,
      [service.id]: nextPosition,
    }));

    router.patch(
      route('services.toggleHome', service.id),
      {},
      {
        preserveScroll: true,
        preserveState: true,
        ...
        onError: () => {
          setOptimisticHomePositions((prev) => {
            const copy = { ...prev };
            if (copy[service.id] !== undefined) {
              delete copy[service.id];
            }
            return copy;
          });
          toast.error('Maximum 3 services sur la page d’accueil');
        },
      }
    );
  };
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="font-semibold"> {selectedService?.name}</span> sera supprimé
              définitivement.
            </AlertDialogDescription>
          </AlertDialogHeader>
