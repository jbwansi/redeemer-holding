        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Textarea
                rows={3}
                value={m.welcome_message}
                onChange={(e) => setMeta({ welcome_message: e.target.value })}
              />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button
              type="button"
              size="sm"
              onClick={() => setMeta({ quick_questions: [...m.quick_questions, ''] })}
            >
            ...
            {m.quick_questions.map((q, i) => (
              <div key={i} className="flex gap-2">
                <Input
                  ...
                  onChange={(e) =>
                    setMeta({
                      quick_questions: m.quick_questions.map((x, idx) =>
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setMeta({ quick_questions: m.quick_questions.filter((_, idx) => idx !== i) })
                  }
                >
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center gap-2 rounded-md border p-3">
              <input
                type="checkbox"
                checked={Boolean(m.lead_capture_enabled)}
                onChange={(e) => setMeta({ lead_capture_enabled: e.target.checked })}
                className="h-4 w-4"
              />
            ...
            <div>
              ...
              <Input
                value={m.lead_prompt}
                onChange={(e) => setMeta({ lead_prompt: e.target.value })}
              />
            </div>
          </CardContent>
        </Card>
              ...
              <div key={i} className="border rounded-lg p-4 space-y-2">
                <div className="flex justify-end">
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    onClick={() => setMeta({ faqs: m.faqs.filter((_, idx) => idx !== i) })}
                  >
                    <Trash2 className="h-4 w-4 text-red-500" />
                  </Button>
                </div>
