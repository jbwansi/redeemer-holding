...
const ShowEvent = ({ event }: any) => {
  ...
  const safeContent = React.useMemo(
    () => DOMPurify.sanitize(displayedEvent.content || ''),
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-6">
            <div>
              <Badge style={{ backgroundColor: displayedEvent.category.color }} className="mb-4">
            ...
            <CardContent className="p-6 space-y-6">
              <div className="flex items-center justify-between">
                <span className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                  {parseInt(displayedEvent.price).toLocaleString()} CHF
                </span>
                <Badge variant="outline" className="text-xs">
                  {displayedEvent.is_full ? 'Complet' : 'Ouvert'}
                </Badge>
              </div>
              ...
              >
                <Link href={route('events.participants', event.slug)}>
                  Voir les inscrits ({displayedEvent.participant_count} /{' '}
                  {displayedEvent.max_participants})
                </Link>
              </Button>
