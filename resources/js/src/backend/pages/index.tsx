...
import { Search, Plus, MoreVertical, Pencil, Trash2, Eye, X, Loader2 } from 'lucide-react';
import {
  Select,
  SelectContent,
  SelectItem,
...
const SettingsContent = ({
  ...
  pages: PageOption[];
  businessSettings: BusinessProps[];
}) => {
  const { data, setData, processing, errors, post } = useForm({
    confidential_page:
      businessSettings?.find((page) => page.type === 'confidential_page')?.value ?? '',
                    ...
                    {(searchQuery || selectedStatus !== 'all') && (
                      <div className="flex flex-wrap gap-2 mt-4">
                        ...
                        {selectedStatus !== 'all' && (
                          <Badge variant="outline" className="gap-1">
                            <span>
                              Statut : {selectedStatus === 'active' ? 'Actives' : 'Inactives'}
                            </span>
                            <X
                              className="w-3 h-3 cursor-pointer"
                              onClick={() => {
                        ...
                        <TableBody>
                          {pages.data.map((page: any) => (
                            <TableRow key={page.id}>
                              ...
                              <TableCell>
                                <Badge variant="outline">{page.viewed}</Badge>
                              </TableCell>
                              <TableCell>
