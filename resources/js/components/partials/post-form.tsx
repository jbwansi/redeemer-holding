...
export function PostForm({ post, categories }: PostFormProps) {
  ...
  const [previewImage, setPreviewImage] = React.useState<string>(post?.featured_image ?? '');
  const safePreviewContent = React.useMemo(
    () => DOMPurify.sanitize(form.data.content || ''),
    [form.data.content]
  );
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Paramètres de publication de l'article</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
          ...
          <div className="flex gap-4">
            <Button
              className=" h-12 w-full rounded-xl text-base font-medium hover:scale-105 transition-transform dark:bg-primary dark:text-white"
              type="button"
              variant="outline"
