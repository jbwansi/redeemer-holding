...
const VisitorChatbot = () => {
    ...
    const configuredQuickQuestions = config?.quick_questions?.length ? config.quick_questions : quickQuestions

    useEffect(() => {
        const loadConfig = async () => {
            try {
                const response = await fetch('/api/chatbot/config', { headers: { 'Accept': 'application/json' } })
                ...
                <div className="fixed bottom-24 right-4 z-[70] w-[360px] max-w-[calc(100vw-2rem)] rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900">
                    ...
                    {Boolean(config?.lead_capture_enabled) && !leadSaved && (
                        <div className="border-t border-slate-200 px-3 py-3 dark:border-slate-700">
                            <p className="text-xs text-slate-600 dark:text-slate-300 mb-2">{config?.lead_prompt || 'Laissez votre email pour etre recontacte rapidement.'}</p>
                            <div className="flex items-center gap-2">
                                <input
                                    type="email"
                                    value={leadEmail}
                                ...
                                />
                                <button onClick={saveLead} className="h-9 rounded-lg bg-slate-900 px-3 text-xs text-white dark:bg-slate-100 dark:text-slate-900">Envoyer</button>
                            </div>
                        </div>
                    )}
