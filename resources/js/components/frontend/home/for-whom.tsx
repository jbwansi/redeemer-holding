...
export default function ForWhom({ title = 'Pour qui ?', subtitle = '', cards = [] }: ForWhomProps) {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {cards.map((card, index) => {
            const Icon = ICON_MAP[card.icon || 'CheckCircle'] ?? LucideIcons.CheckCircle;

            const CardWrapper = card.cta_url ? Link : 'div';
            const wrapperProps = card.cta_url ? { href: card.cta_url } : {};
        ...
        <div className="mt-12 text-center">
          <p className="mx-auto mb-6 max-w-2xl text-base text-gray-600 dark:text-gray-300">
            Vous vous reconnaissez dans l'un de ces profils ? Découvrez l'accompagnement le plus
