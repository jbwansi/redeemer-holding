...
const TrainingCard = ({ formation }: any) => {
  const isPast = new Date(formation.end_date) < new Date();
  const isOngoing = formation.is_ongoing;

  const formatTrainingDate = (dateString: any, includeYear = true) => {
  ...
  const formatTrainingTime = (dateString: any) => {
    const date = new Date(dateString);
    ...
    if (formation.available_seats <= 5) {
      return (
        <div className="flex items-center text-red-600 dark:text-red-400 font-medium">
          <Users className="w-4 h-4 mr-2" />
          <span>
            Plus que {formation.available_seats} place{formation.available_seats > 1 ? 's' : ''}
          </span>
        </div>
      );
    }
        ...
        <div className="md:w-1/3 relative overflow-hidden h-48 md:h-auto">
          ...
          <div className="absolute bottom-0 left-0 right-0 md:hidden bg-gradient-to-t from-black/80 via-black/45 to-transparent py-4 px-4">
            <div className="flex items-center text-white">
              ...
              <span className="text-sm">{formatTrainingDate(formation.start_date, false)}</span>
            </div>
          </div>
        </div>
          ...
          <div className="mb-auto">
            <div className="hidden md:flex flex-wrap gap-4 text-sm text-gray-500 dark:text-gray-400 mb-3">
              <div className="flex items-center">
                ...
                <span>
                  Du {formatTrainingDate(formation.start_date)} au{' '}
                  {formatTrainingDate(formation.end_date)}
                </span>
              </div>
              ...
              <div className="flex items-center">
                ...
                <span>
                  {formatTrainingTime(formation.start_date)} -{' '}
                  {formatTrainingTime(formation.end_date)}
                </span>
              </div>
            </div>
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-3 group-hover:text-primary dark:group-hover:text-primary transition-colors duration-300">
              <Link href={route('formations.details', formation.slug)}>{formation.title}</Link>
            </h3>
