...
export function Navbar({ title }: NavbarProps) {
    ...
    const [unreadNotifications, setUnreadNotifications] = useState<number>(notifications?.unread_count ?? 0);
    ...
    useEffect(() => {
        ...
        if (q.length < 2) {
            setGlobalResults({ users: [], events: [], formations: [], posts: [], services: [] });
        ...
        const timeout = window.setTimeout(async () => {
            try {
                setIsSearching(true);
                const response = await fetch(`${route('dashboard.search.global')}?q=${encodeURIComponent(q)}`, {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        Accept: 'application/json',
                    },
    ...
    return (
        <header className="sticky top-0 z-50 w-full border-b border-border/50 bg-background/80 backdrop-blur-xl supports-[backdrop-filter]:bg-background/70">
                ...
                <div className="flex items-center justify-end gap-1.5 md:gap-3">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" size="icon" className="relative rounded-xl border-border/70 bg-card/60 hover:bg-card h-8 w-8 md:h-10 md:w-10">
                        ...
                        <DropdownMenuContent align="end" className="w-80">
                            ...
                            {unreadNotifications > 0 && (
                                <DropdownMenuItem className="justify-center font-medium p-0">
                                    <Link href={route('profile.notifications.read-all')} method="post" as="button" className="w-full py-2">
            ...
            <CommandDialog open={open} onOpenChange={setOpen}>
                ...
                />
                <CommandList>
                    <CommandEmpty>{isSearching ? 'Recherche en cours...' : 'Aucun résultat trouvé.'}</CommandEmpty>
                    {searchQuery.trim().length >= 2 && (
                        <>
                                ...
                                <CommandGroup heading="Utilisateurs">
                                    {globalResults.users.map((item) => (
                                        <CommandItem key={`user-${item.id}`}>
                                            <Link href={item.url} className="flex w-full items-center justify-between" onClick={() => setOpen(false)}>
                            ...
                            {globalResults.formations.length > 0 && (
                                <CommandGroup heading="Formations">
                                    {globalResults.formations.map((item) => (
